...
- (void)buildMainMenuCard
{
    ...
    NSButton *droneButton = [self makeMenuButtonWithTitle:@"Drone Mode" action:@selector(toggleDroneMode:) primary:NO];
    ...
    NSStackView *buttonStack = [NSStackView stackViewWithViews:@[startButton, randomButton, droneButton, hudButton, supportButton, quitButton]];
...
- (void)applicationWillTerminate:(NSNotification *)notification
{
    ...
    if (_cursorHiddenForDrone) {
        [NSCursor unhide];
        _cursorHiddenForDrone = NO;
    }
}

- (void)installInputMonitors
{
    __weak AFAppDelegate *weakSelf = self;
    _keyDownMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                                            handler:^NSEvent *(NSEvent *event) {
        return [weakSelf handleDroneKeyEvent:event down:YES];
    }];
    _keyUpMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyUp
                                                          handler:^NSEvent *(NSEvent *event) {
        return [weakSelf handleDroneKeyEvent:event down:NO];
    }];
    _mouseMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskMouseMoved |
                     NSEventMaskLeftMouseDragged |
                     NSEventMaskRightMouseDragged |
                     NSEventMaskOtherMouseDragged
                                                          handler:^NSEvent *(NSEvent *event) {
        AFAppDelegate *strongSelf = weakSelf;
        if (!strongSelf || ![strongSelf->_renderer isDroneModeEnabled] || !strongSelf->_window.isKeyWindow) {
            return event;
        }
        [strongSelf->_renderer applyDroneMouseDeltaX:event.deltaX deltaY:event.deltaY];
        return nil;
    }];
    _mouseDownMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskLeftMouseDown
        ...
        [strongSelf->_renderer fireDroneLaser];
        return nil;
    }];
}

- (NSEvent *)handleDroneKeyEvent:(NSEvent *)event down:(BOOL)down
{
    if (!event) {
        return event;
    }
    ...
    if (event.keyCode == 53 && [_renderer isDroneModeEnabled]) {
        if (down) {
            [self setDroneModeActive:NO];
        }
        return nil;
    }

    if (![_renderer isDroneModeEnabled] || !_window.isKeyWindow) {
        return event;
    }
    ...
    [self syncDroneMovement];
    return nil;
}

- (void)syncDroneMovement
{
    [_renderer setDroneMovementForward:_droneForward
                                   back:_droneBack
                                   left:_droneLeft
                                  right:_droneRight
                                     up:_droneUp
...
- (void)setDroneModeActive:(BOOL)active
{
    [_renderer setDroneModeEnabled:active];
    if (active) {
        [self hideLaunchOverlay];
        [_window makeFirstResponder:_metalView];
        if (!_cursorHiddenForDrone) {
            [NSCursor hide];
            _cursorHiddenForDrone = YES;
        }
    } else {
        _droneForward = NO;
        _droneBack = NO;
...
- (void)buildMenus
{
    ...
    _droneMenuItem = [compositionMenu addItemWithTitle:@"Drone Mode"
                                                action:@selector(toggleDroneMode:)
                                         keyEquivalent:@"d"];
    _droneMenuItem.target = self;
...
- (void)refreshMenuState
{
    ...
    _droneMenuItem.state = [_renderer isDroneModeEnabled] ? NSControlStateValueOn : NSControlStateValueOff;
}
...
{
    (void)sender;
    if ([_renderer isDroneModeEnabled]) {
        [self setDroneModeActive:NO];
    }
...
- (void)toggleDroneMode:(id)sender
{
    (void)sender;
    [self setDroneModeActive:![_renderer isDroneModeEnabled]];
}
...
- (void)showAbout:(id)sender
{
    ...
    alert.informativeText = @"A procedural C++/Metal generative art studio with GPU particles, shader layers, mutation controls, PNG export, first-person drone mode, and experimental MetalFX support.\n\nDrone controls: W/A/S/D thrust, mouse steers, Space/E rises, Q/C descends, click fires the laser, Esc exits.\n\nCredit / support: https://buymeacoffee.com/chcofficial";
    [alert runModal];
}
